...
// Lưu trạng thái điểm số, số lần thêm từ mới, trạng thái thua và trạng thái trò chơi cho từng mode
let modeStates = {
  classic: { 
    score: 0, 
    hasPlayed: false, 
...
document.getElementById("backToHomeButton").onclick = () => {
  ...
  if (gameMode !== "team") {
    const modeKey = gameMode === "classic" ? "classic" : gameMode === "timed" ? "timed" : "themed";
    // Đặt lại trạng thái khi quay về trang chủ
    modeStates[modeKey].score = score;
    modeStates[modeKey].hasPlayed = false; // Đặt lại hasPlayed để hiển thị resumeModal khi quay lại
